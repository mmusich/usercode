...
class diowGenerator
{
  public:
    ...
    void setIconSize(uint32_t size=300) { iconsize_=size; }
...
diowGenerator::diowGenerator() {
    username_ = "musich";
    title_ = "b tagging input variables comparison";
    outhtml_ = "index.html";
    path_ = ".";
    textcolor_ = "000066";
    bgcolor_ = "FFFFFF";
...
void diowGenerator::addItem(const char* filename) {
   ...
   TString outputhistoname(filename);
   TString append(filename);
  
   if(outputhistoname.Contains("_udsg")){
     append="udsg jets";
   } else if (outputhistoname.Contains("_b")){
     append="b jets"; 
   } else {
     append="all jets"; 
   }
