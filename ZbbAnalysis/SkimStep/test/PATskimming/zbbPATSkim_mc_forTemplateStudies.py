...
process.source = cms.Source("PoolSource",
   fileNames = cms.untracked.vstring(
    ...
    #'rfio:/castor/cern.ch/user/m/musich/ZBBTest/Skim_Ele_ZplusJet.root'
    #'file:/tmp/castello/DYjet_test.root'
    'file:/tmp/emiglior/C0D6C43D-F615-E011-ADBA-E0CB4E19F9A1.root'
    )
)
...
## next two modules just to decrease the output rate in case of b-tag discriminant template studies
#from PhysicsTools.PatAlgos.selectionLayer1.selectedPatCandidates_cff import *
process.selectedHighPtPatJets = cms.EDFilter("PATJetSelector",
                                     src = cms.InputTag("patJets"),
                                     cut = cms.string('pt > 20.0 && abs(eta) < 2.4')
                                     ) 

process.countHighPtPatJets = cms.EDFilter("CandViewCountFilter",
                                  src = cms.InputTag("selectedHighPtPatJets"),
                                  minNumber = cms.uint32(1)
                                  )

process.jetFilter = cms.Sequence(process.selectedHighPtPatJets*process.countHighPtPatJets)
