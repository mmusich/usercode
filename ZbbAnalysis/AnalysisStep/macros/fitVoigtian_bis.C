{
  ...
  TFile fl("analyzePAT_MC_ZlJets_All.root");   
  TFile fc("analyzePAT_MC_ZcToLL_All.root");
  TFile fb("analyzePAT_MC_Zb5fToLL_All.root");
  
  fl.cd("finaldistros_ssvhem/JetBTag");
  Z_mass=(TH1F*)JetBTag_MllNOMASSCUT->Clone();
  Z_mass->Rebin(2);

  fc.cd("finaldistros_ssvhem/JetBTag");
  Zc_mass=(TH1F*)JetBTag_MllNOMASSCUT->Clone();
  Zc_mass->Rebin(2);
  
  fb.cd("finaldistros_ssvhem/JetBTag");
  Zb_mass=(TH1F*)JetBTag_MllNOMASSCUT->Clone();
  Zb_mass->Rebin(2);
