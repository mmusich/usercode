...
//                            |_|                
  ...
  if(muons->size()>1){
	...
	if((muon1->charge()*muon2->charge())<0){ 
	  ...
	  fill("AllDiMuDeltaPhi",deltaR);
	  fill("AllDiMuDeltaR",deltaPhi);
	}
      }						 
    }
  }
  ...
  if(muons->size()>1){
	...
	if ( muon2->pt() > 10. && 
	  ...
	  if((muon1->charge()*muon2->charge())<0){ 
	    ...
	    double deltaR = ROOT::Math::VectorUtil::DeltaR(muon1->momentum(),muon2->momentum());
	    double deltaPhi = ROOT::Math::VectorUtil::DeltaPhi(muon1->momentum(),muon2->momentum());
	    fill("PreselDiMuDeltaPhi",deltaR);
	    fill("PreselDiMuDeltaR",deltaPhi);
	  }
	}
      }
      }
...
//                                  |_|                
  ...
  if(electrons->size()>1){
	...
	if((electron1->charge()*electron2->charge())<0){
	  ...
	  fill("AllDiEleDeltaPhi",deltaR);
	  fill("AllDiEleDeltaR",deltaPhi);
	}
      }						 
    }
  }
	...
	for (electron2 = electron1+1; electron2 != electrons->end(); electron2++) {
	  ...
	  if (  electron2->pt() > 10.0 && abs(electron2->eta()) < 2.5 && (electron2->isEE() || electron2->isEB()) && !electron2->isEBEEGap() && electron2->electronID("eidVBTFRel80") == 7 ){
	    ...
	    if((electron1->charge()*electron2->charge())<0){ 
	      ...
	      double deltaR = ROOT::Math::VectorUtil::DeltaR(electron1->momentum(),electron2->momentum());
	      double deltaPhi = ROOT::Math::VectorUtil::DeltaPhi(electron1->momentum(),electron2->momentum());
	      fill("PreselDiEleDeltaPhi",deltaR);
	      fill("PreselDiEleDeltaR",deltaPhi);
	    }
	  }
	}
      }
...
PatBasicAnalyzer::beginJob()
{
  ...
  histContainer_["leadingjet_phi"  ]=fs->make<TH1F>("leadingjet_phi"  , "#phi (Jet);#phi (Leading Jet)"       ,   60, -3.2,   3.2);
  ...
  histContainer_["subleadingjet_phi"  ]=fs->make<TH1F>("subleadingjet_phi"  , "#phi (Jet);#phi (Subleading Jet)"       ,   60,-3.2,   3.2);
  ...
  histContainer_["muon_chi2"]= fs->make<TH1F>("muon_chi2", "muon #chi2/ndof; muon #chi2/ndof", 100,   0.,  10.);
  histContainer_["muon_trackerhits"]=fs->make<TH1F>("muon_trackerhits", "muon Trk hits;trk hits", 40,  -0.5,39.5);
