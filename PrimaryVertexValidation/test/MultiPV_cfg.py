...
process.load("Alignment.OfflineValidation.ALCARECOTkAlMinBias_prompt_110325_cff");
...
process.GlobalTag.globaltag = "GR_P_V14::All"  # take your favourite
...
process.trackerAlignment = cms.ESSource("PoolDBESSource",CondDBSetup,
                                        connect = cms.string('sqlite_file:/afs/cern.ch/cms/CAF/CMSALCA/ALCA_TRACKERALIGN/PayLoads/Offline_Reprocessing_Tags/TwistFree/alignments_MPCRAFT11fromTwistFree.db'),
...
if isDA:  
     process.PVValidation = cms.EDAnalyzer("MultiPVValidation",
                                           ...
                                           OutputFileName = cms.string("PVValidation2011GeoMultiPV.root"),
                                           ...
                                           TkClusParameters=cms.PSet(algorithm=cms.string('DA'),
                                                                     TkDAClusParameters = cms.PSet(coolingFactor = cms.double(0.8),  #  slow annealing
                                                                                                   Tmin = cms.double(9.0),           #  freezeout temperature
                                                                                                   vertexSize = cms.double(0.05),    #  ~ resolution / sqrt(Tmin)
                                                                                                   d0CutOff = cms.double(3.),        #  downweight high IP tracks 
                                                                                                   dzCutOff = cms.double(4.)         #  outlier rejection after freezeout
                                                                                                   )
                                                                     )
                                           )
     ...
     process.PVValidation = cms.EDAnalyzer("MultiPVValidation",
                                           ...
                                           TkClusParameters = cms.PSet(algorithm   = cms.string('gap'),
                                                                       TkGapClusParameters = cms.PSet(zSeparation = cms.double(0.2)  ## 4.2 mm max separation betw. clusters
                                                                                                      ) 
                                                                       )
                                           )
