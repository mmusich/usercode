/* 
 ...
 usage:
 $ root -l
 $ root [0] .L PlotPVValidationWebPage.C++
 $ root [1] PlotPVValidationWebPage("filename1.root=legendentry1,filename2.root=legendentry2", .... , number_of_files,"YourCut") 
...
void PlotPVValidationWebPage(TString namesandlabels,Int_t nOfFiles, TCut theCut);
void fitResiduals(TH1 *hist);
void DoubleGausFitResiduals(TH1 *hist);
std::pair<Double_t,Double_t> returnMeanAndSigma(TF1 *totalgaus);
Double_t returnMeanError(TF1 *totalgaus);
...
void PlotPVValidationWebPage(TString namesandlabels,Int_t nOfFiles, TCut theCut){
  ...
  for(Int_t i=0; i<nplots; i++){
    for(Int_t j=0; j < nOfFiles; j++){
      ...
      histosdxy[j][i] = new TH1F(treeleg1,histotitledxy,nbins_,-1500,1500);
      ...
      histosdz[j][i] = new TH1F(treeleg2,histotitledz,nbins_,-1500,1500);
      ...
      histosEtadxy[j][i] = new TH1F(treeleg1eta,histotitledxyeta,nbins_,-1500,1500);
      ...
      histosEtadz[j][i] = new TH1F(treeleg2eta,histotitledzeta,nbins_,-1500,1500);
  ...
  Double_t lowedge=-0.5;
   ...
   for(Int_t j=0; j < nOfFiles; j++) { 
    ...
    histoMeansdxy[j]->GetYaxis()->SetRangeUser(-50,50);
    ...
    histoMeansdz[j]->GetYaxis()->SetRangeUser(-70,70);
    ...
    histoSigmasdxy[j]->GetYaxis()->SetRangeUser(0,310);
    ...
    histoSigmasdz[j]->GetYaxis()->SetRangeUser(0,510);
    ...
    histoSigmasdxyEta[j]->GetYaxis()->SetRangeUser(0,310);
    ...
    histoSigmasdzEta[j]->GetYaxis()->SetRangeUser(0,1000);
   ...
   for(Int_t j=0; j < nOfFiles; j++) { 
	...
	histosdz[j][i]->Draw("sames");
      ...
      //DoubleGausFitResiduals(histosdz[j][i]);
	...
	//meansdz[j][i] = returnMeanAndSigma(tmpsdz[j][i]).first;
	//sigmasdz[j][i]= returnMeanAndSigma(tmpsdz[j][i]).second;

	meansdzError[j][i]=(tmpsdz[j][i])->GetParError(1);
	//meansdzError[j][i]=returnMeanError(tmpsdz[j][i]);
	...
	histosEtadz[j][i]->Draw("sames");
      ...
      //DoubleGausFitResiduals(histosEtadz[j][i]);   
	...
	//meansdzEta[j][i] = returnMeanAndSigma(tmpsdzEta[j][i]).first;
	//sigmasdzEta[j][i]= returnMeanAndSigma(tmpsdzEta[j][i]).second;

	meansdzEtaError[j][i]=(tmpsdzEta[j][i])->GetParError(1);	
	//meansdzEtaError[j][i]=returnMeanError(tmpsdzEta[j][i]);
  ...
  pt->SetTextFont(42);
  pt->SetTextAlign(11);
  TText *text1 = pt->AddText("CMS preliminary 2010");
  ...
  for(Int_t j=0; j < nOfFiles; j++) { 
    ...
    if(j==0){
      histoMeansdxy[j]->SetMarkerSize(1.2);
      histoMeansdxy[j]->Draw("e1");
    }
    else  histoMeansdxy[j]->Draw("e1sames");
    ...
    if(j==0){
      histoMeansdz[j]->SetMarkerSize(1.2);
      histoMeansdz[j]->Draw("e1");
    }
    else  histoMeansdz[j]->Draw("e1sames");
...
void DoubleGausFitResiduals(TH1 *hist)
{
  ...
  TF1 func1("gaus1", "gaus", mean - 1*sigma, mean + 1*sigma); 	    
  TF1 func2("gaus2", "gaus", mean - 4*sigma, mean + 4*sigma); 	    
  TF1 func("tmp", "gaus(0)+gaus(3)", mean - 4*sigma, mean + 4*sigma);
  ...
  if(hist->GetEntries()>20) {
    ...
    if (0 == hist->Fit(&func,"QNR")) { // N: do not blow up file by storing fit!
      ...
      func.SetRange(mean - 4*sigma, mean + 4*sigma);
      // I: integral gives more correct results if binning is too wide
      // L: Likelihood can treat empty bins correctly (if hist not weighted...)
      if (0 == hist->Fit(&func, "Q0LR")) {
	if (hist->GetFunction(func.GetName())) { // Take care that it is later on drawn:
...
std::pair<Double_t,Double_t> returnMeanAndSigma(TF1 *totalgaus){
  ...
  Double_t n1= totalgaus->GetParameter(0);
  Double_t m1= totalgaus->GetParameter(1);
  Double_t s1= totalgaus->GetParameter(2);
  Double_t n2= totalgaus->GetParameter(3);
  Double_t m2= totalgaus->GetParameter(4);
  Double_t s2= totalgaus->GetParameter(5);

  Double_t theMean = (n1*m1 + n1*m2)/(n1+n2);
  Double_t theSigma = TMath::Sqrt(((n1*(m1*m1 +s1*s1) + n2*(m2*m2 + s2*s2))/(n1+n2)) - theMean*theMean);	 
  
  fitResult.first  = theMean;
  fitResult.second = theSigma;
...
Double_t returnMeanError(TF1 *totalgaus){
  ...
  Double_t dn1= totalgaus->GetParError(0);
  Double_t dm1= totalgaus->GetParError(1);
  Double_t dn2= totalgaus->GetParError(3);
  Double_t dm2= totalgaus->GetParError(4);

  Double_t theMeanError = TMath::Sqrt( (n1/(n1+n2))*(n1/(n1+n2))*dm1*dm1 + (n2/(n1+n2))*(n2/(n1+n2))*dm2*dm2 + (m1*(n1+n2) - (n1*m1+n2*m2))/((n1+n2)*(n1+n2))* (m1*(n1+n2) - (n1*m1+n2*m2))/((n1+n2)*(n1+n2))*dn1*dn1 +  (m2*(n1+n2) - (n1*m1+n2*m2))/((n1+n2)*(n1+n2))* (m1*(n1+n2) - (n1*m1+n2*m2))/((n1+n2)*(n1+n2))*dn2*dn2);
