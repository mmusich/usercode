/* 
 ...
 usage:
 $ root -l
 $ root [0] .L PlotPVValidation_forDevel.C++
 $ root [1] PlotPVValidation_forDevel("filename1.root=legendentry1,filename2.root=legendentry2", .... , number_of_files,"YourCut") 
...
void PlotPVValidation_forDevel(TString namesandlabels,Int_t nOfFiles,TCut theCut,TString theLeg,Bool_t useBS=false,Bool_t verbose=false);
...
void MainLoop(TString namesandlabels,Int_t nOfFiles){
  ...
  Double_t minRunNumber_=evTree->GetMinimum("RunNumber");
  Double_t maxRunNumber_=evTree->GetMaximum("RunNumber");

  for(Double_t i=minRunNumber_;i<maxRunNumber_;i++){
    char theRunCutstring[128];
    sprintf(theRunCutstring,"(RunNumber>=%.fi) && (RunNumber<%.fi)",i,i+1);
    std::cout<<theRunCutstring<<std::endl;
    TCut theRunCut=theRunCutstring;
    PlotPVValidation_forDevel(namesandlabels,nOfFiles,theRunCut,"",false);
  }
}

void PlotPVValidation_forDevel(TString namesandlabels,Int_t nOfFiles,TCut theCut,TString theLeg,Bool_t useBS,Bool_t verbose){
  ...
  for(Int_t j=0; j < nOfFiles; j++) {
    ...
    trees[j] = (TTree*)fin->Get("PVValidation/tree");

    // Retrieve labels

    TObjString* legend = (TObjString*)LabelList->At(j);
    LegLabels[j] = legend->String();
    cout<<"PlotPVValidation_forDevel() label["<<j<<"] "<<LegLabels[j]<<endl;
    
  }

  //Double_t theRun = LegLabels[0].Atof();
  Double_t theRun = theLeg.Atof();
  
  char theRunCutstring[128];
  //sprintf(theRunCutstring,"(RunNumber>=%.f)&&(RunNumber<%.f)",theRun,theRun+1);
  sprintf(theRunCutstring,"RunNumber==%.f",theRun);
  //std::cout<<theRunCutstring<<std::endl;
  TCut theRunCut=theRunCutstring;

  TCut theDefaultCut= "hasRecVertex==1"+theRunCut;
  std::cout<<"PlotPVValidation_forDevel() theDefaultCut: "<<theDefaultCut<<endl; 

  //TString label = LegLabels[nOfFiles-1];
  //label.ReplaceAll(" ","_");

  TString label = theLeg;
 
  // check for out files if exixting remove, and then create it
  TSystemDirectory dir(".", ".");
  TString theOutFileName_ =  "histos"+label+".root";
  const char *ext= theOutFileName_;
  ...
  if (files) {
	...
	TString processline = ".! rm histos"+label+".root";
	gROOT->ProcessLine(processline.Data());
	
	std::cout <<"PlotPVValidation_forDevel()" << fname.Data() << " already existing! Removing it!"<< endl;
      }
    }
  }
  ...
  for(Int_t j=0; j < nOfFiles; j++) { 
   ...
   for(Int_t j=0; j < nOfFiles; j++) { 
     std::cout<<"PlotPVValidation_forDevel() Running on file: "<<LegLabels[j]<<std::endl;
     for(Int_t i=0; i<nplots; i++){
       if(verbose) {
	 //std::cout<<"PlotPVValidation_forDevel() Fitting the : "<<i<<"/"<<nplots<<" bin ";
   ...
   TPaveText *pt = new TPaveText(0.58,0.70,0.75,0.85,"NDC");
   pt->SetFillColor(10);
   pt->SetTextColor(1);
   //pt->SetTextSize(0.05);
   pt->SetTextFont(42);
   pt->SetTextAlign(11);
   TText *text1 = pt->AddText("CMS Tracker Alignment 2011");
   text1->SetTextSize(0.04);
   TText *text2 = pt->AddText("Pb-Pb #sqrt{s_{NN}}=2.76 TeV");
   text2->SetTextSize(0.04); 
   TText *text3 = pt->AddText("run :"+theLeg);
...
{ 
  ...
  if (0 == hist->Fit(&func1,"QR")) {
    ...
    cout<<"PlotPVValidation_forDevel() fit Up done!"<<endl;
  }
...
void FitDLine(TH1 *hist)
{
  ...
  if (0 == hist->Fit(&func2,"QR")) {
    ...
    cout<<"PlotPVValidation_forDevel() fit Down done!"<<endl;
  } 
}
